 ...
 export default function MyEvents() {
   ...
   useEffect(() => {
    ...
    const currentUserName = storedUser ? JSON.parse(storedUser).name : "";
        const myEvents = data.filter((event:EventItem) => event.creatorName === currentUserName);
        const sortedEvents: EventItem[] = [...myEvents].sort(
         ...
         {filteredEvents.length > 0 ? (
           filteredEvents.map((event) => (
             ...
             >
               ...
               <p className="mt-1 text-sm m-2">Created by {event.creatorName}</p>
               <div className="flex items-center space-x-6">
               <Link href={`/Events/${event.id}`}>
               <button type="button" className="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">View</button>
             </Link>
             ...
             <form onSubmit={handleSubmit}>
               <input type="hidden" name="id" value={event.id} />
                 <button type="submit" className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</button>
              </form>
